...
void CFBXLoader::LoadMaterial(FbxSurfaceMaterial* pMtrl)
{
	...
	pMtrlInfo->strEmvTex = GetMaterialTexture(pMtrl, FbxSurfaceMaterial::sEmissive);
}
...
void CFBXLoader::RetargetBone(const std::string& strKey)
{
	CSkeleton* pSkeleton = GET_SINGLE(CResourceManager)->FindSkeleton(strKey);

	if (!pSkeleton)
		return;

	std::vector<PFBXBONE> vecNotFound;
	int iCount = pSkeleton->GetBoneCount();
	std::vector<PFBXBONE> vecBones(iCount);

	size_t iSize = m_vecBones.size();

	for (int i = 0; i < iCount; i++)
	{
		PBONE pBone = pSkeleton->GetBone(i);

		PFBXBONE pNewBone = new FBXBONE;
		vecBones[i] = pNewBone;
		pNewBone->iDepth = pBone->iDepth;
		pNewBone->iParentIndex = pBone->iParentIndex;
		pNewBone->strName = pBone->strName;
	}
	...
	for (int i = 0; i < iSize; i++)
	{
		...
		{
			if (m_vecBones[i]->strName == vecBones[j]->strName)
			{
				bFound = true;
			}
		}
		...
		{
			PFBXBONE pNewBone = new FBXBONE;
			pNewBone->iDepth = m_vecBones[i]->iDepth;

			for (int j = 0; j < vecBones.size(); j++)
			{
				if (m_vecBones[m_vecBones[i]->iParentIndex]->strName == vecBones[j]->strName)
				{
					pNewBone->iParentIndex = j;
				}
			}

			pNewBone->strName = m_vecBones[i]->strName;
