...
#include "Matrix.h"
	...
	typedef struct _tagVector3
	{
		...
		// +
		const _tagVector3 operator+ (const _tagVector3& v)	const;
		const _tagVector3 operator+ (const DirectX::XMVECTOR& v)	const;
		const _tagVector3 operator+ (float f)	const;
		...
		const _tagVector3 operator - ()	const;
