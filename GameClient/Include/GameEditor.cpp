...
BEGIN_MESSAGE_MAP(CGameEditorApp, CWinApp)
	...
	ON_COMMAND(ID_32772, &CGameEditorApp::OnSave)
	ON_COMMAND(ID_32771, &CGameEditorApp::OnLoad)
END_MESSAGE_MAP()
...
{
	const TCHAR strFilter[] = TEXT("장면파일(*.ges)|*.ges|모든파일(*.*)|*.*||");

	CFileDialog filedlg(TRUE, TEXT("*.ges"),TEXT("Scene"), 
		OFN_HIDEREADONLY, strFilter);

	filedlg.m_ofn.lpstrInitialDir = GET_SINGLE(CPathManager)->FindPath(DATA_PATH);

	if (filedlg.DoModal() == DI_NOTATTACHED)
	{
		CString strFileName = filedlg.GetFileName();
		std::string strFullPath = CT2CA(strFileName);

		CWnd* pWnd = AfxGetMainWnd();

		((CMainFrame*)pWnd)->ClearCreateDlg();

		GET_SINGLE(CSceneManager)->Load(strFullPath.c_str());
	}
}
...
void CGameEditorApp::OnSave()
{
	...
	CFileDialog filedlg(FALSE, TEXT("*.ges"), TEXT("Scene"),
		OFN_OVERWRITEPROMPT, strFilter);

	filedlg.m_ofn.lpstrInitialDir = GET_SINGLE(CPathManager)->FindPath(DATA_PATH);
	INT_PTR llResult = filedlg.DoModal();
	if (llResult == DI_NOTATTACHED)
	{
		CString strFileName = filedlg.GetFileName();
		std::string strFullPath = CT2CA(strFileName);

		GET_SINGLE(CSceneManager)->Save(strFullPath.c_str());
	}
}
