...
void TestCase::doTests(dtNavMesh* navmesh, dtNavMeshQuery* navquery, std::vector<unsigned int>& vecPolys, std::vector<float>& vecStraight, int& _iPoly, int& _iStraight)
{
	if (!navmesh || !navquery)
		return;
	...
	for (Test* iter = m_tests; iter; iter = iter->next)
	{
		...
		if (iter->type == TEST_PATHFIND)
		{
			...
			float* pResult = nullptr;
			int iVisit = 0;
			//navquery->moveAlongSurface(startRef, iter->spos, iter->epos, &filter, pResult, polys, &iVisit, 256);
			...
			if (iter->npolys)
			{
				...
				/*for (int i = 0; i < iter->nspos; i++)
				{
					printf_s("%.2f", iter->spos[i])
				}*/
			}
	...
	for (Test* iter = m_tests; iter; iter = iter->next)
	{
		int iPoly = iter->npolys;
		_iPoly = iPoly;
		vecPolys.resize(iPoly);
		memcpy_s(&vecPolys[0], iPoly * 4, iter->polys, iPoly * 4);
		int iStraight = iter->nstraight;
		_iStraight = iStraight;
		vecStraight.resize(iStraight * 3);
		memcpy_s(&vecStraight[0], iStraight* 3 * 4, iter->straight, iStraight * 3 * 4);
		const int total = iter->findNearestPolyTime + iter->findPathTime + iter->findStraightPathTime;

		for (int i = 0; i < iPoly; i++)
		{
			printf("%d ", vecPolys[i]);
		}
		printf("\n");
		for (int i = 0; i < iStraight; i++)
		{
			printf("%.2f ", vecStraight[i]);
		}
		printf("\n");
...
void TestCase::TestPath(dtNavMesh* navmesh, dtNavMeshQuery* navquery, const Vector3& vStart, const Vector3& vEnd, std::vector<float>& vecStraight, int& _iStraight)
{
	if (!navmesh || !navquery)
		return;
	...
	// Find start points
	dtPolyRef startRef, endRef;
	float nspos[MAX_POLYS] = {};
	float nepos[MAX_POLYS] = {};
	navquery->findNearestPoly(&vStart.x, polyPickExt, &filter, &startRef, nspos);
	navquery->findNearestPoly(&vEnd.x, polyPickExt, &filter, &endRef, nepos);
	...
	int iVisit = 0;
	int npolys = 0;
	navquery->findPath(startRef, endRef, &vStart.x, &vEnd.x, &filter, polys, &npolys, MAX_POLYS);

	// Find straight path
	int nstraight = 0;
	if (npolys)
	{
		navquery->findStraightPath(&vStart.x, &vEnd.x, polys, npolys,
			straight, 0, 0, &nstraight, MAX_POLYS);
	}
	_iStraight = nstraight;
	// Copy results
	//if (npolys)
	//{
	//	iter->polys = new dtPolyRef[iter->npolys];
	...
	if (nstraight)
	{
		vecStraight.resize(nstraight * 3);
		memcpy(&vecStraight[0], straight, sizeof(float) * 3 * nstraight);
	}
