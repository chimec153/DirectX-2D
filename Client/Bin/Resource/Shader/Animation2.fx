...
StructuredBuffer<tFrameTrans>	g_pFrame	:	register(t13);
StructuredBuffer<matrix>	g_pOffset	:	register(t14);
StructuredBuffer<tFrameTrans>	g_pChangeFrame	:	register(t15);
RWStructuredBuffer<matrix>	g_pFinal : register(u0);

[numthreads(256, 1, 1)]
void CS(int3 ID	:	SV_DispatchThreadID)
{
	if (g_iBoneCount <= ID.x)
		return;

	uint iCurFrame = g_iAnimFrameCount * ID.x + g_iCurrentFrame;
	int iNext = g_iNextFrame;

	if (iNext >= g_iAnimFrameCount)
		iNext = 0;

	uint iNextFrame =  g_iAnimFrameCount* ID.x + iNext;

	float4 vPos = lerp(g_pFrame[iCurFrame].vMove, g_pFrame[iNextFrame].vMove, g_fRatio);
	float4 vScale = lerp(g_pFrame[iCurFrame].vScale, g_pFrame[iNextFrame].vScale, g_fRatio);
	float4 qRot = QuternionLerp(g_pFrame[iCurFrame].qRot, g_pFrame[iNextFrame].qRot, g_fRatio);

	if (g_iChange)
	{
		vPos = lerp(vPos, g_pChangeFrame[g_iChangeFrame  * ID.x].vMove, g_fChangeRatio);
		vScale = lerp(vScale, g_pChangeFrame[g_iChangeFrame * ID.x].vScale, g_fChangeRatio);
		qRot = QuternionLerp(qRot, g_pChangeFrame[g_iChangeFrame* ID.x].qRot, g_fChangeRatio);
	}

	matrix matBone = (matrix)0.f;
	MatrixAffineTransformation(vScale, float4(0.f, 0.f, 0.f, 1.f), qRot, vPos, matBone);

	matrix matOffset = transpose(g_pOffset[ID.x]);

	g_pFinal[ID.x] = mul(matOffset, matBone);
}